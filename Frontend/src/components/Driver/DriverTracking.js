...
const DriverTracking = () => {
  ...
  useEffect(() => {
    ...
    console.log("Joinging booking room by driver", bookingId);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          setDriverLocation(currentLocation); // Update state

          // Emit current driver location to the socket
          if (socketRef.current) {
            socketRef.current.emit("driverLocationUpdate", {
              roomId: bookingId,
              driverLocation: currentLocation, // Ensure `currentLocation` has correct values
            });
          }
        },
        (err) => {
  ...
  // Use an effect to send location updates every 2 minutes
  useEffect(() => {
    const intervalId = setInterval(updateDriverLocation, 1000); // 120000 ms = 2 min
